...
public class InteractiveCommandParser implements Runnable {
		...
		while (true) {
			...
			if (c.accessToDoOperations) {
				...
				if (command.contains("store")) {
						...
						if (needed.length() == 0) {
							...
							try {
								if (mergeSplit.splitFile(new File(needed))) {
									...
									System.out
											.println("[INFO] Total Chunks Created: "
													+ c.totalNumberOfChunk);
									System.out
											.println("[INFO] Starting Requesting Controller");

									c.fileName = needed;
									c.accessToDoOperations = false;
									StartRequestingController startRequestingController = new StartRequestingController(
											c, c.totalNumberOfChunk, needed);
									// startRequestingController.chunkNumber =
									// i+1;
									// startRequestingController.FileName =
									// needed;
									// startRequestingController.reqClient = c;
									// byte[] x = {(byte) 1};
					...
					System.out.println("[INFO]Updating Is Under COnstruction..");
					System.out.println("[INFO]Sorry");
	...
	private void startRequestingController(Client c, int myIDAsClient,
		...
		c.connectionToController.sender.sendData(requestingReadToController
				.getByte());
	}
